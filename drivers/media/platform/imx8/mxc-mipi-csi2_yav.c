			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	/* if specific IRS238xC, */
	if (fmt->format.width == 224) {
...
static int csi_set_ctrl_exposure(struct mxc_mipi_csi2_dev *csi, int exp)
{
//	struct csi_ctrls *ctrls = &csi->ctrls;
	int ret = 0;
...
static int csi_set_ctrl_contrast(struct mxc_mipi_csi2_dev *sensor, int value)
{
	int ret=0;
...
static inline struct mxc_mipi_csi2_dev *to_csi_dev(struct v4l2_subdev *sd)
{
	return container_of(sd, struct mxc_mipi_csi2_dev, sd);
}

static inline struct v4l2_subdev *ctrl_to_sd(struct v4l2_ctrl *ctrl)
{
	return &container_of(ctrl->handler, struct mxc_mipi_csi2_dev, ctrls.handler)->sd;
}


static int csi_s_ctrl(struct v4l2_ctrl *ctrl)
{
	struct v4l2_subdev *sd = ctrl_to_sd(ctrl);
	struct mxc_mipi_csi2_dev *csidev = to_csi_dev(sd);
	int ret;
	printk("%s enter\n", __func__);
	...
	case V4L2_CID_EXPOSURE:
		ret = csi_set_ctrl_exposure(csidev, ctrl->val);
		break;		
/*	case V4L2_CID_AUTOGAIN:
	...
	case V4L2_CID_CONTRAST:
		ret = csi_set_ctrl_contrast(csidev, ctrl->val);
		break;
...
static const struct v4l2_ctrl_ops csi_ctrl_ops = {
	.s_ctrl = csi_s_ctrl,
};


static int csi2_init_controls(struct mxc_mipi_csi2_dev *csidev)
{
	const struct v4l2_ctrl_ops *ops = &csi_ctrl_ops;
	struct csi_ctrls *ctrls = &csidev->ctrls;
...
static int mipi_csi2_probe(struct platform_device *pdev)
{
	...
	ret = csi2_init_controls(csi2dev);
	if (ret)
		mutex_destroy(&csi2dev->lock);
