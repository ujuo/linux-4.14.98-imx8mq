...
static int mx6s_videoc_s_ctrl(struct file *file, void *fh,
        struct v4l2_control *ctrl)
{
	struct mx6s_csi_dev *csi_dev = video_drvdata(file);
	struct v4l2_subdev *sd = csi_dev->sd;
    struct v4l2_ctrl *handler;
    int ret;
    
    printk("%s enter\n", __func__);
    printk("%s \n", &sd->name);
    handler = v4l2_ctrl_find(sd->ctrl_handler, ctrl->id);
    if(handler){
    	ret = v4l2_ctrl_s_ctrl(handler,ctrl->value);
    }
    
    if(ret < 0){
    	pr_err("%s: failed to subdev s_crl\n", __func__);
    }
    printk("%s exit\n", __func__);
    return ret;
}
...
static const struct v4l2_ioctl_ops mx6s_csi_ioctl_ops = {
	...
	.vidioc_s_ctrl          = mx6s_videoc_s_ctrl,
};
