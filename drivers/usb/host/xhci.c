			...
			struct usb_device *udev, int enable)
{
	...
	if (hcd->speed >= HCD_USB3 || !xhci->hw_lpm_support ||
			!udev->lpm_capable){
		printk("hcd->speed %d >= HCD_USB3 || !xhci->hw_lpm_support %d || !udev->lpm_capable %d\n",
			     hcd->speed, xhci->hw_lpm_support, udev->lpm_capable);
		return -EPERM;
	}
	
	if (!udev->parent || udev->parent->parent ||
			udev->descriptor.bDeviceClass == USB_CLASS_HUB){
		printk("udev->parent 0x%X || udev->parent->parent 0x%X || dev->descriptor.bDeviceClass %d\n", 
			     udev->parent, udev->parent->parent, udev->descriptor.bDeviceClass);
		return -EPERM;
	}

	if (udev->usb2_hw_lpm_capable != 1){
		printk("udev->usb2_hw_lpm_capable %d\n", udev->usb2_hw_lpm_capable);
		return -EPERM;
	}
